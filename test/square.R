...
plot(log10(lambda), res21, pch=4, xlab='log_10(lambda)', ylab='residual', col=1)
grid()
abline(v=log10(best_lambda21), lty=2, col='red')
lines(-12:3, res21, col=1)
points(log10(best_lambda21), res21[10]/2+res21[11]/2, pch=19, col='red')
points(-12:3, res22, pch=4, col=2)
lines(-12:3, res22, col=2)
points(log10(best_lambda21), res22[10]/2+res22[11]/2, pch=19, col='red')
lines(-12:3, res23, col=3)
points(-12:3, res23, pch=4, col=3)
...
points(log10(best_lambdaCov1), resCov1[11], pch=19, col='red')
...
points(-12:3, resCov5, pch=1, col=4)
...
tcovE=microbenchmark(smooth.FEM(observations=dataCov, FEMbasis=FEMbasis, covariates=cov, lambda=best_lambdaCov1))

boxplot(log(tA$time),log(tB$time),log(tC$time), names=c('fdaPDE','mass lumping','lambda'),col=c('grey',2,3), ylab='log(time)')
