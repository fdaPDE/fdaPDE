...
template<UInt ORDER>
std::vector<UInt> projection<ORDER,1,2>::computeNodePatch(UInt id_node) const
{
    constexpr UInt NNODES = how_many_nodes(ORDER,1);
    std::vector<UInt> patch_node;
...
template<UInt ORDER>
std::vector<Point<2> > projection<ORDER,1,2>::computeProjection(){
    ...
    for(UInt i=0; i<num_points; ++i){
        ...
        for(UInt n=0; n<mesh_.num_nodes(); ++n){
            Point<2> current_point = mesh_.getPoint(n);
            if(deData_[i].dist2(current_point) < dist2){
                pos_min = n;
                dist2 = deData_[i].dist2(current_point);
            }
        ...
        for(auto elem : computeNodePatch(pos_min)){
            ...
            Point<2> proj = mesh_.getElement(elem).computeProjection(deData_[i]);
