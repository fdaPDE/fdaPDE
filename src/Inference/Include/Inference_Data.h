...
class InferenceData
{
	private:
		// Type of analysis required
  		std::string test_Type            	= "not-defined";			//!< Values: not-defined [default], p-value, power
  		std::string interval_Type		= "not-defined";			//!< Values: not-defined [default], one-at-the-time, bonferroni
  		std::string implementation_Type		= "wald"; 				//!< Values: wald [default], sandwich, permutational
  		bool exact_Inference			= false;				//!< If true the inverse of E matrix of woodbury decomposition of the system is computed exatctly
		// Parameter needed
  		std::vector<bool> coeff_Inference	={};					//!< If position j is true, the j-th covariate is taken into account in the inferential analysis
  		std::vector<Real> beta_0		={}; 					//!< Values for the null hypostesis, if test_Type != not-defined
  		std::vector<Real> beta_1 		={};					//!< Values for the alternative hypotesis,  if test_Type != not-defined
  		Real inference_Level			=0; 					//!< Significance level for confidence intervals if interval_Type != not-defined
  		bool definition				=false;					//!< Defines whether the analysis needs to be caried out or not
	...
	public:
 	 ...
 	 //Setters
  		...
  		inline void set_coeff_inference(const std::vector<bool> & coeff_inf){coeff_Inference = coeff_inf;};		//!< Setter for coeff_Inference \param coeff_Inference_ new coeff_Inference
  		inline void set_beta_0(const std::vector<Real> & beta_0_){beta_0 = beta_0_;};					//!< Setter for beta0 \param beta0_ new beta0
  		inline void set_beta_1(const std::vector<Real> & beta_1_){beta_1 = beta_1_;};					//!< Setter for beta1 \param beta1_ new beta1
  ...
  inline std::vector<bool> get_coeff_inference() const{return this->coeff_Inference;};		//!< Getter for coeff_Inference \return coeff_Inference
  inline std::vector<Real> get_beta_0() const{return this->beta_0;};				//!< Getter for beta0 \return beta0
  inline std::vector<Real> get_beta_1() const{return this->beta_1;};				//!< Getter for beta1 \return beta1
