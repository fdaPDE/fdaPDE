...
public:
  ...
  Speckman_Solver(Speckman_Solver && rhs):inverter(std::move(rhs.inverter)), inf_car(rhs.inf_car), B(std::move(rhs.B)), Lambda2(std::move(rhs.Lambda2)), is_Lambda2_computed(rhs.is_Lambda2_computed), V(std::move(rhs.V)), is_V_computed(rhs.is_V_computed), WLW_dec(std::move(rhs.WLW_dec)), is_WLW_computed(rhs.is_WLW_computed){}; //Definition move constructor
