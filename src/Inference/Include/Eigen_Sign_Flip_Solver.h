...
public:
  ...
  Eigen_Sign_Flip_Solver(const std::string & exact_inference_, const Inference_Carrier<InputHandler> & inf_car_):Solver_Base<InputHandler>(exact_inference_, inf_car_){}; 
  Eigen_Sign_Flip_Solver(Eigen_Sign_Flip_Solver & rhs) = delete; //The default copy constructor is deleted
  inline Eigen_Sign_Flip_Solver(Eigen_Sign_Flip_Solver && rhs):Partial_res_H0(std::move(rhs.Partial_res_H0)), B(std::move(rhs.B)), Lambda(std::move(rhs.Lambda)), is_Lambda_computed(rhs.is_Lambda_computed) {this->inverter = std::move(rhs.inverter); this->inf_car = rhs.inf_car; this->exact_inference = rhs.exact_inference;}; //Definition of the move constructor
  ...
  void print_for_debug(void) const override;
};
