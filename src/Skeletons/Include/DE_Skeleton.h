...
SEXP DE_skeleton(SEXP Rdata, SEXP Rorder, SEXP Rscaling, SEXP Rfvec, SEXP RheatStep, SEXP RheatIter, SEXP Rlambda, SEXP Rnfolds, SEXP Rnsim, SEXP RstepProposals,
	SEXP Rtol1, SEXP Rtol2, SEXP Rprint, SEXP Rmesh, SEXP Rsearch, SEXP Rinference,
	const std::string & step_method, const std::string & direction_method, const std::string & preprocess_method)
{
	...
	DataProblem<ORDER, mydim, ndim> dataProblem(Rdata, Rorder, Rscaling, Rfvec, RheatStep, RheatIter, Rlambda, Rnfolds, Rnsim, RstepProposals, Rtol1, Rtol2, Rprint, Rsearch, Rmesh, Rinference);
	...
	SEXP result = NILSXP;
	result = PROTECT(Rf_allocVector(VECSXP, 7 + 5));
    ...
    SET_VECTOR_ELT(result, 5, Rf_allocVector(REALSXP, g_lower.size()));
    SET_VECTOR_ELT(result, 6, Rf_allocVector(REALSXP, g_upper.size()));
	...
	if(dataProblem.getSearch()==2){
		...
		SET_VECTOR_ELT(result, 7, Rf_allocVector(INTSXP, 1)); //tree_header information
		int *rans7 = INTEGER(VECTOR_ELT(result, 7));
		rans7[0] = dataProblem.getMesh().getTree().gettreeheader().gettreelev();

		SET_VECTOR_ELT(result, 8, Rf_allocVector(REALSXP, ndim*2)); //tree_header domain origin
		Real *rans8 = REAL(VECTOR_ELT(result, 8));
		for(UInt i = 0; i < ndim*2; i++)
			rans8[i] = dataProblem.getMesh().getTree().gettreeheader().domainorig(i);

		SET_VECTOR_ELT(result, 9, Rf_allocVector(REALSXP, ndim*2)); //tree_header domain scale
		Real *rans9 = REAL(VECTOR_ELT(result, 9));
		for(UInt i = 0; i < ndim*2; i++)
			rans9[i] = dataProblem.getMesh().getTree().gettreeheader().domainscal(i);


		UInt num_tree_nodes = dataProblem.getMesh().num_elements()+1; //Be careful! This is not equal to number of elements
		SET_VECTOR_ELT(result, 10, Rf_allocMatrix(INTSXP, num_tree_nodes, 3)); //treenode information
		int *rans10 = INTEGER(VECTOR_ELT(result, 10));
		for(UInt i = 0; i < num_tree_nodes; i++)
			rans10[i] = dataProblem.getMesh().getTree().gettreenode(i).getid();

		for(UInt i = 0; i < num_tree_nodes; i++)
			rans10[i + num_tree_nodes*1] = dataProblem.getMesh().getTree().gettreenode(i).getchild(0);

		for(UInt i = 0; i < num_tree_nodes; i++)
			rans10[i + num_tree_nodes*2] = dataProblem.getMesh().getTree().gettreenode(i).getchild(1);

		SET_VECTOR_ELT(result, 11, Rf_allocMatrix(REALSXP, num_tree_nodes, ndim*2)); //treenode box coordinate
		Real *rans11 = REAL(VECTOR_ELT(result, 11));
		for(UInt j = 0; j < ndim*2; j++)
		{
			for(UInt i = 0; i < num_tree_nodes; i++)
				rans11[i + num_tree_nodes*j] = dataProblem.getMesh().getTree().gettreenode(i).getbox().get()[j];
		}
	}
