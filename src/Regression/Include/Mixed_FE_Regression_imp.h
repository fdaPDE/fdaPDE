...
{
	if (diagPreconditioned || JacobiPreconditioned)
		return preconditioner.asDiagonal() * b;

	else if (blockPreconditioned)
	{
...
{
	if (massLumping)
		return (lumpedMass.cwiseInverse().asDiagonal() * b);

	MatrixXr bp(b);
	return matrixNoCovdec_.solve(preconditionRHS(bp));
}
...
MatrixXr MixedFERegressionBase<InputHandler>::system_solve(const Eigen::MatrixBase<Derived> & b)
{
	...
	//if (massLumping)
	//	MatrixXr x1 = lumpedMass.cwiseInverse().asDiagonal() * b;

	//else
	//{
	//	MatrixXr bp(b);
	//	MatrixXr x1 = matrixNoCovdec_.solve(preconditionRHS(bp));
	//}
	...
	//	// Resolution of the system matrixNoCov * x3 = U * x2
	//	if (massLumping)
	//		MatrixXr x3 = lumpedMass.cwiseInverse().asDiagonal() * (U_ * x2);

	//	else
	//	{
	//		MatrixXr bp(U_*x2);
	//		MatrixXr x3 = matrixNoCovdec_.solve(preconditionRHS(bp));
	//	}
	//	//MatrixXr x3 = solver.system_solve(U_ * x2);
	//	x1 -= x3;
	//}
