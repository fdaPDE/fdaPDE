...
class DirectionBase{
  ...
  public:
    //! A constructor
    DirectionBase(const FunctionalProblem<ORDER, mydim, ndim>& fp): funcProblem_(fp){};
    //! A destructor.
    virtual ~DirectionBase(){};
    //! A pure virtual clone method.
    virtual std::unique_ptr<DirectionBase<ORDER, mydim, ndim>> clone() const = 0;
...
template<UInt ORDER, UInt mydim, UInt ndim>
class DirectionGradient : public DirectionBase<ORDER, mydim, ndim>{
  public:
    //! A delegating constructor.
    DirectionGradient(const FunctionalProblem<ORDER, mydim, ndim>& fp):
    DirectionBase<ORDER, mydim, ndim>(fp){};
    //! Clone method overridden.
    std::unique_ptr<DirectionBase<ORDER, mydim, ndim>> clone() const override;
...
template<UInt ORDER, UInt mydim, UInt ndim>
class DirectionBFGS : public DirectionBase<ORDER, mydim, ndim>{
  private:
  ...
  public:
    //! A constructor.
    DirectionBFGS(const FunctionalProblem<ORDER, mydim, ndim>& fp, UInt k):
    DirectionBase<ORDER, mydim, ndim>(fp), HInit_(MatrixXr::Identity(k, k)), HOld_(MatrixXr::Identity(k, k)), updateH_(false){};
    //! A copy constructor: it just creates a DirectionBFGS object with the same features of rhs but it initializes the matrices.
    DirectionBFGS(const DirectionBFGS<ORDER, mydim, ndim>& rhs);
    //! Clone method overridden.
    std::unique_ptr<DirectionBase<ORDER, mydim, ndim>> clone() const override;
