...
###
mesh = create.mesh.1D.vertices(vertices, edges, delta[3])
nnodes = mesh$nnodes
delta=c(0.025, 0.0125, 0.00625)


{
mesh = create.mesh.1D.vertices(vertices, edges, 0.03)
nodes = mesh$nodes
nnodes = mesh$nnodes
FEMbasis = create.FEM.basis.1D(mesh) 
...
sigma.dx = 0.045 #NODO 8 
sigma.sx = 0.085 # NOD0 37
  
source.dx = cbind(mesh.fdaPDE$nodes[8,])
source.sx = cbind(mesh.fdaPDE$nodes[37,])
}

aux.sx = function(x,y,seg,tp){
  sigma = sigma.sx
  h = 5*sigma
  source = source.sx
  points_ = cbind(x,y)
  ...
  m_ = (mesh.fdaPDE$nodes[5,2] - mesh.fdaPDE$nodes[4,2]) / ( mesh.fdaPDE$nodes[5,1]- mesh.fdaPDE$nodes[4,1])
  idx.ok = which( abs(- points_[,2] + m_*(points_[,1]-mesh.fdaPDE$nodes[4,1]) + mesh.fdaPDE$nodes[4,2] )
                           < 10*.Machine$double.eps )
  
  #idx.ok = which(  sqrt( (points_[,2] - source[2])^2  +
   #                      (points_[,1]- source[1])^2 ) < h ) 
  
  for( i in idx.ok){
    
    delta = sqrt( (points_[i,1]-source[1])^2 +
                        (points_[i,2]-source[2])^2 )
    if(delta<h){
...
aux = function(x,y,seg,tp){
  ...
  res = 0.5*( aux.sx(x,y,seg,tp) + aux.dx(x,y,seg,tp) )
...
coef.sx = aux.sx(mesh.fdaPDE$nodes[,1], mesh.fdaPDE$nodes[,2])
R_plot_graph.ggplot2(FEM(coef.sx,FEMbasis.fdaPDE))

coef.dx = aux.dx(mesh.fdaPDE$nodes[,1], mesh.fdaPDE$nodes[,2])
R_plot_graph.ggplot2(FEM(coef.dx,FEMbasis.fdaPDE))
...
ntest_ = "-test-1-delta-var-300-obs-PARTE-2"
