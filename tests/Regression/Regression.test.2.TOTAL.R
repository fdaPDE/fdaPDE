...
for( k in 1:K){
  ...
  for( i in 1:N){
    ...
    for( j in 1:M){
      ...
      rmse.fdaPDE[[k]][j,i] = norm(beta_ex - output.fdaPDE$solution$beta[1], type="2")
  ...
  save(nobs,M,N,K, delta, nnodes, rmse, rmse.fdaPDE,
...
today_ = Sys.Date()
ntest_ = "-test-2-part2-COMPLETE-2" # "-test.1.3" 
#today_ = "2021-11-15"
