...
#set.seed(42)                    #test ???
#a = rnorm(1, mean=0, sd=1)      #test ??? 
#b = rnorm(1, mean=1.5, sd=0.05) #test ???

a=1  #test 1
b=1  #test 1

aux = function(x, y, seg, tp) { a*cos((2*x+y)/4) +  b*((x+y))^2 }

integral_ex = integrate_f( FEM(aux(mesh.fdaPDE$nodes[,1],mesh.fdaPDE$nodes[,2]),FEMbasis.fdaPDE) )

aux = function(x, y, seg, tp) {1/integral_ex*( cos((2*x+y)/4) +  ((x+y))^2) }
...
#load(file = save.file)
  ...
  RMSE.beta.1 - RMSE.fdaPDE.beta.1
  RMSE.beta.2 - RMSE.fdaPDE.beta.2

### ggplot2 ###
  
  #loading data
  today_ = "2021-11-15"
  file.name = paste("Regression-",today_,sep="")
  save.file = paste("/home/aldo/Scrivania/fdaPDE-DATA/Regression/",file.name,".RData",sep="")
  load(save.file)

  ### saving test.1.4 imgs ###
  today_ = "2021-12-03"
  #today_ = Sys.Date()
  ntest_ = "-test.1.3"
  file.name = paste("Regression-",today_,ntest_,sep="")
...
ggplot(data_frame.1) +
  ...
  labs(x="",y="",fill="",title=TeX( "M \times RMSE(\\beta_{1})^{2}") )+
...
ggplot(data_frame.1) +
  ...
  labs(x="observations",y="",fill="",title=TeX( "M \times MISE"))+
  theme(plot.title = element_text(hjust = 0.5))
...
for(i in 1:N){
   ...
   print( ggplot(data_frame.tmp, aes(x=rmse.beta.1_,y=rmse.fdaPDE.beta.1_)) +
    ...
    labs(x="R", y="fdaPDE", 
         title= TeX("Scatter $ M \times RMSE(\\beta_{1})^{2}$"),
...
print(  ggplot(data_frame.tmp, aes(x=rmse.beta.2_,y=rmse.fdaPDE.beta.2_)) +
    geom_point() +
    labs(x="R", y="fdaPDE", 
         title= TeX("Scatter $ M \times RMSE(\\beta_{2})^{2}$"),
         subtitle= paste("nobs = ",nobs[i],sep=""))+
    theme(plot.title = element_text(hjust=0.5)) )
