...
if(!dir.exists(test_path("../data/GSR-PDE/test_20"))){
  dir.create(test_path("../data/GSR-PDE/test_20"))  

options(warn=-1)
foldername <- test_path("../data/GSR-PDE/test_20/")
...
invisible(capture.output(sol_ref <- smooth.FEM(location = loc, observations = as.numeric(response), FEMbasis =FEMbasis, covariates = NULL,
                                 ...
                                 lambda=lambda)))
save(sol_ref, file=paste0(foldername,"/test_20_1.RData"))

#### Test 20.2: With exact GCV
invisible(capture.output(sol_ref <- smooth.FEM(location = loc, observations = as.numeric(response), FEMbasis =FEMbasis, covariates = NULL,
                                 max.steps=15, fam=FAMILY, mu0=NULL, scale.param=NULL,
                                 lambda = lambda, lambda.selection.criterion = 'grid', 
                                 DOF.evaluation = 'exact', lambda.selection.lossfunction = 'GCV')))
save(sol_ref, file=paste0(foldername,"/test_20_2.RData"))
}
