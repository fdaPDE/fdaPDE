...
if(!dir.exists(test_path("../data/GSR-PDE/test_18"))){
  dir.create(test_path("../data/GSR-PDE/test_18"))  
...
invisible(capture.output(sol_ref <- smooth.FEM(location = NULL, observations = as.numeric(response), FEMbasis =FEMbasis, covariates = desmat,
                        max.steps=15, family=FAMILY, mu0=NULL, scale.param=NULL,
                        lambda = lambda)))
save(sol_ref, file=paste0(foldername,"/test_18_1.RData"))
...
save(sol_ref, file=paste0(foldername,"/test_18_2.RData"))
}
