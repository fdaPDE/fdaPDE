if(!dir.exists(test_path("../data/tGSR-PDE")))
  dir.create(test_path("../data/tGSR-PDE"))

if(!dir.exists(test_path("../data/tGSR-PDE/test_26"))){
  dir.create(test_path("../data/tGSR-PDE/test_26"))  
...
invisible(capture.output(sol_ref <- smooth.FEM.time(time_mesh = time_mesh, time_locations = time_locations,
                              locations = loc, observations = response, FEMbasis = FEMbasis, 
                              covariates = W, 
                              lambdaS = lambdaS[1], lambdaT =  lambdaT[1],
                              family = FAMILY, FLAG_PARABOLIC = T)))
save(sol_ref, file=paste0(foldername,"/test_26_1.RData"))
...
invisible(capture.output(sol_ref <- smooth.FEM.time(time_mesh = time_mesh, time_locations = time_locations,
                              ...
                              family = FAMILY, FLAG_PARABOLIC = T)))
save(sol_ref, file=paste0(foldername,"/test_26_2.RData"))
...
save(sol_ref, file=paste0(foldername,"/test_26_3.RData"))
...
invisible(capture.output(sol_ref <- smooth.FEM.time(time_mesh = time_mesh, time_locations = time_locations,
                              ...
                              lambdaS = lambdaS, lambdaT =  lambdaT,
                              family = FAMILY, FLAG_PARABOLIC = F)))
save(sol_ref, file=paste0(foldername,"/test_26_4.RData"))
}
