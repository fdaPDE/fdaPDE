...
if(!dir.exists(test_path("../data/GSR-PDE/test_19"))){
  dir.create(test_path("../data/GSR-PDE/test_19"))  
...
invisible(capture.output(sol_ref <- smooth.FEM(location = loc, observations = as.numeric(response), FEMbasis = FEMbasis, covariates = NULL,
                                 ...
                                 scale.param=NULL,
                                 lambda = lambda)))
save(sol_ref, file=paste0(foldername,"/test_19_1.RData"))
...
save(sol_ref, file=paste0(foldername,"/test_19_2.RData"))
}
