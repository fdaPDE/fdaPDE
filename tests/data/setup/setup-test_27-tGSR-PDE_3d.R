...
if(!dir.exists(test_path("../data/tGSR-PDE/test_27"))){
  dir.create(test_path("../data/tGSR-PDE/test_27"))  
...
#### Test 27.1: without GCV 
invisible(capture.output(sol_ref <- smooth.FEM.time(locations = NULL, observations = response, FEMbasis =FEMbasis, 
...
save(sol_ref, file=paste0(foldername,"/test_27_1.RData"))

#### Test 27.2: with exact GCV
invisible(capture.output(sol_ref <- smooth.FEM.time(locations = NULL, observations = response, FEMbasis =FEMbasis, 
                              covariates = NULL, time_mesh = time_locations, time_locations = time_locations,
                              max.steps=15, family=FAMILY,
...
save(sol_ref, file=paste0(foldername,"/test_27_2.RData"))
}
