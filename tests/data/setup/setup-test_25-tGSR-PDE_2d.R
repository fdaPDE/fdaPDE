...
if(!dir.exists(test_path("../data/tGSR-PDE/test_25"))){
  dir.create(test_path("../data/tGSR-PDE/test_25"))  
...
invisible(capture.output(sol_ref <- smooth.FEM.time(time_mesh = time_mesh, time_locations = time_locations,
                              ...
                              lambdaS = lambdaS[2], lambdaT = lambdaT[2],
                              family = FAMILY, FLAG_PARABOLIC = T)))
save(sol_ref, file=paste0(foldername,"/test_25_1.RData"))
...
invisible(capture.output(sol_ref <- smooth.FEM.time(time_mesh = time_mesh, time_locations = time_locations,
                              ...
                              lambdaS = lambdaS[2], lambdaT = lambdaT[2],
                              family = FAMILY, FLAG_PARABOLIC = F)))
save(sol_ref, file=paste0(foldername,"/test_25_2.RData"))
}
