test_that("GSR-PDE 2d areal data",{
  options(warn=-1)
  ...
  invisible(capture.output(sol <- smooth.FEM(observations = as.numeric(response), FEMbasis = FEMbasis, covariates = desmat,
                                                 max.steps=15, fam=FAMILY, mu0=NULL, scale.param=NULL,
                                                 incidence_matrix = t(incidence_matrix), areal.data.avg = FALSE,
                                                 lambda = lambda)))
  load(file=paste0(foldername,"/test_23_1.RData"))
  ...
  load(file=paste0(foldername,"/test_23_2.RData"))
  expect_equal( max(abs((sol$fit.FEM$coeff-sol_ref$fit.FEM$coeff))) < tol, TRUE);
  expect_equal( max(abs((sol$solution$beta-sol_ref$solution$beta))) < tol, TRUE);
})