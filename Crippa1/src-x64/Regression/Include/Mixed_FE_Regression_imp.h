...
{
	return;
	//Eigen::JacobiSVD<SpMat> temp(matrixNoCov_);
	//auto K_ = temp.singularValues()[0];
}
...
void MixedFERegressionBase<InputHandler>::buildMatrixNoCov(const SpMat & NWblock, const SpMat & SWblock,  const SpMat & SEblock)
{
	...
	//for(UInt k=0; k<NWblock.outerSize(); ++k)
	//	for(UInt j=0; j<NWblock.innerSize(); ++j)
	//	{
	//		coeff elem(k, j, NWblock.coeff(k,j)+SWblock.coeff(j,k));
	//		tripletAll.push_back(elem);
	//	}
	//for(UInt k=0; k<SEblock.outerSize(); ++k)
	//	for(UInt j = 0; j < SEblock.innerSize(); ++j)
	//	{
	//		coeff elem(k, j, SEblock.coeff(k, j) + SWblock.coeff(k,j));
	//		tripletAll.push_back(elem);
	//	}
	//for (UInt k = 0; k < SWblock.outerSize(); ++k)
	//	for (UInt j = 0; j < SWblock.innerSize(); ++j)
	//	{
	//		coeff elem(k, j+nnodes, 0);
	//		tripletAll.push_back(elem);
	//	}
	//for (UInt k = 0; k < SWblock.outerSize(); ++k)
	//	for (UInt j = 0; j < SWblock.innerSize(); ++j)
	//	{
	//		coeff elem(k+nnodes, j, 0);
	//		tripletAll.push_back(elem);
	//	}
...
MatrixXr MixedFERegressionBase<InputHandler>::system_solve(const Eigen::MatrixBase<Derived> & b)
{
	...
	//Eigen::DiagonalPreconditioner<Real> temp(matrixNoCov_);
	//MatrixXr matrixTot_ = MatrixXr::Zero(2*N_*M_, 2*N_*M_);

	//MatrixXr matrixTot_ = computeLumpedMatrix(matrixNoCov_);
	...
	//	MatrixXr Ginv_ = Gdec_.solve(I);
	//	MatrixXr matrixOnlyCov_ = U_*Ginv_*V_;

	//	matrixOnlyCov_.conservativeResizeLike(MatrixXr::Zero(2*N_*M_,2*N_*M_));
	//	matrixTot_ += matrixOnlyCov_;
	//	//matrixTot_ += computeLumpedMatrix(matrixOnlyCov_);
	//}
	//matrixTot_ += matrixNoCov_;

	//Eigen::DiagonalPreconditioner<Real> temp(computeLumpedMatrix(matrixTot_));
	//Eigen::LeastSquareDiagonalPreconditioner<Real> prec(matrixTot_);

	//MatrixXr x1 = matrixNoCovdec_.solve(prec.solve(b));

	//MatrixXr x1 = temp.solve(b);
	...
	//	x1 -= matrixNoCovdec_.solve(prec.solve(U_ * x2));
...
MatrixXr MixedFERegressionBase<InputHandler>::computeLumpedMatrix(const MatrixType & matrixInit)
//(const SpMat & matrixInit)
{
	UInt ncol = matrixInit.cols();
	UInt nrow = matrixInit.rows();
	MatrixXr diag_(nrow, ncol);

	//UInt ncol = matrixInit.rows();
	//UInt nrow = matrixInit.cols();
	//MatrixXr One_ = MatrixXr::Ones(nrow, ncol);
	//MatrixXr temp = matrixInit * One_;
	//MatrixXr diag_(ncol, nrow);
	//diag_.diagonal()=temp_.diagonal();
	...
	for (UInt k = 0; k < nrow; ++k)
	{
		...
		{
			t += matrixInit.coeff(k, j);
		}
		diag_.coeffRef(k,k) = t;
	}
	return diag_;
}
	...
	public:
		...
		void preapply(const MeshHandler<ORDER,mydim,ndim> & mesh)
		{
			typedef EOExpr<Stiff> ETStiff; Stiff EStiff; ETStiff stiff(EStiff);
		    	MixedFERegressionBase<RegressionData>::preapply<ORDER,mydim,ndim, IntegratorSpace, IntegratorTime, SPLINE_DEGREE, ORDER_DERIVATIVE>
				(stiff, ForcingTerm(std::vector<Real>(1)), mesh);
		}
};
	...
	public:
		...
		void preapply(const MeshHandler<ORDER,mydim,ndim> & mesh)
		{
			...
			else
			{
				...
				MixedFERegressionBase<RegressionDataElliptic>::preapply<ORDER,mydim,ndim, IntegratorSpace, IntegratorTime, SPLINE_DEGREE, ORDER_DERIVATIVE>
					(c*mass+stiff[K]+dot(b,grad), ForcingTerm(std::vector<Real>(1)), mesh);
			}
		}
};
