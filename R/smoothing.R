  ...
  if(family == "gaussian")
  {
    ...
    }else if(class(FEMbasis$mesh) == 'mesh.2D' & !is.null(PDE_parameters) & space_varying == TRUE)
    {
      ...
      bigsol = CPP_smooth.FEM.PDE.sv.basis(locations = locations, observations = observations, FEMbasis = FEMbasis,
        ...
        optim = optim, lambda = lambda, DOF.stochastic.realizations = DOF.stochastic.realizations, DOF.stochastic.seed = DOF.stochastic.seed,
        DOF.matrix = DOF.matrix, GCV.inflation.factor = GCV.inflation.factor, lambda.optimization.tolerance = lambda.optimization.tolerance,
        R_Inference_Data_Object=R_Inference_Data_Object) # further check for inference consistency will be performed deeper in the code
      numnodes = nrow(FEMbasis$mesh$nodes)
    }else if(class(FEMbasis$mesh) == 'mesh.2.5D')
    {
      bigsol = NULL
      ...
      for(i in 1:length(R_Inference_Data_Object@type)){
        if(R_Inference_Data_Object@interval[i]!=0){
          ...
          if(R_Inference_Data_Object@type[i]==1){
            inference$CI$wald = c(inference$CI$wald, ci)
          }
          else if(R_Inference_Data_Object@type[i]==2){
            inference$CI$speckman = c(inference$CI$speckman, ci)
          }
          else if(R_Inference_Data_Object@type[i]==3){
            inference$CI$eigen_sign_flip = c(inference$CI$eigen_sign_flip, ci)
          }
        }
        ...
        if(R_Inference_Data_Object@type[i]==1){
          inference$p_values$wald = c(inference$p_values$wald, p_values)
        }
        else if(R_Inference_Data_Object@type[i]==2){
          inference$p_values$speckman = c(inference$p_values$speckman, p_values)
        }
        else if(R_Inference_Data_Object@type[i]==3){
          inference$p_values$eigen_sign_flip = c(inference$p_values$eigen_sign_flip, p_values)
        }
        }
       }
