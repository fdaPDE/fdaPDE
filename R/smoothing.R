    ...
    if(R_Inference_Data_Object@definition==1){
      ...
      for(i in 1:length(R_Inference_Data_Object@type)){ # each element is a different inferential setting
        if(R_Inference_Data_Object@interval[i]!=0){ # Intervals requested by this setting, adding them to the right implementation position
          ...
          if(R_Inference_Data_Object@component[i]!=2){ # intervals for beta were requested
        ...
        if(R_Inference_Data_Object@component[i]!=1){ # test on f was requested
          ...
          p_value = numeric()
          if(R_Inference_Data_Object@type[i]==3 || R_Inference_Data_Object@type==4){ # (eigen-)sign-flip p-value is already computed in the cpp code
