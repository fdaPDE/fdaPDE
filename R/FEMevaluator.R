...
eval.FEM <- function(FEM, locations = NULL, incidence_matrix = NULL, search = "tree", bary.locations = NULL)
{
  ...
  if(is(FEMbasis$mesh, "mesh.2.5D") && search ==3)
    stop("2.5D search must be either 'tree' or 'naive'")
  if(is(FEMbasis$mesh, "mesh.1.5D") && search ==3)
                          ...
                          incidence_matrix = NULL, lambdaS = 1, lambdaT = 1, search = "tree", bary.locations = NULL)
{
  ...
  if (FLAG_TENSORIZE) {
    ...
    if (!FLAG_AREAL_EVALUATION) {
      ...
      check.2D =   is(FEMbasis$mesh, "mesh.2D") && ncol(incidence_matrix)!=nrow(FEM.time$FEMbasis$mesh$triangles)
      check.2.5D = is(FEMbasis$mesh, "mesh.2.5D") && ncol(incidence_matrix)!=nrow(FEM.time$FEMbasis$mesh$triangles)
      check.3D =   is(FEMbasis$mesh, "mesh.3D")   && ncol(incidence_matrix)!=nrow(FEM.time$FEMbasis$mesh$tetrahedrons)
      check.1.5D = is(FEMbasis$mesh, "mesh.1.5D") && ncol(incidence_matrix)!=nrow(FEM.time$FEMbasis$mesh$edges)
