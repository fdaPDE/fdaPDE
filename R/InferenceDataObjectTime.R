                                         ...
                                         n_flip = 1000)
{
  ...
  if(sum(component == "parametric")==length(component)){
    if(!is.null(locations) || !is.null(locations_indices) || !is.null(time_locations))
      warning("locations or time locations are provided but not used, since inference is requested only on the paramteric component")
    locations_indices = NULL
    ...
    if(is.null(locations) & is.null(locations_indices)){
      ...
      # other dimensional checks on 'locations_indices' will be performed later, inside checkInferenceParametersTime
    }
    if(is.null(time_locations))
