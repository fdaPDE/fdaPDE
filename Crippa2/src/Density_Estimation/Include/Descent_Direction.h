...
class DirectionBase{
  ...
  public:
    //! A constructor
    DirectionBase(const FunctionalProblem<Integrator, Integrator_noPoly, ORDER, mydim, ndim>& fp): funcProblem_(fp){};
    //! A destructor.
    virtual ~DirectionBase(){};
    //! A pure virtual clone method.
    virtual std::unique_ptr<DirectionBase<Integrator, Integrator_noPoly, ORDER, mydim, ndim>> clone() const = 0;
...
template<typename Integrator, typename Integrator_noPoly, UInt ORDER, UInt mydim, UInt ndim>
class DirectionGradient : public DirectionBase<Integrator, Integrator_noPoly, ORDER, mydim, ndim>{
  public:
    //! A delegating constructor.
    DirectionGradient(const FunctionalProblem<Integrator, Integrator_noPoly, ORDER, mydim, ndim>& fp):
    DirectionBase<Integrator, Integrator_noPoly, ORDER, mydim, ndim>(fp){};
    //! Clone method overridden.
    std::unique_ptr<DirectionBase<Integrator, Integrator_noPoly, ORDER, mydim, ndim>> clone() const override;
...
template<typename Integrator, typename Integrator_noPoly, UInt ORDER, UInt mydim, UInt ndim>
class DirectionBFGS : public DirectionBase<Integrator, Integrator_noPoly, ORDER, mydim, ndim>{
  private:
  ...
  public:
    //! A constructor.
    DirectionBFGS(const FunctionalProblem<Integrator, Integrator_noPoly, ORDER, mydim, ndim>& fp, UInt k):
    DirectionBase<Integrator, Integrator_noPoly, ORDER, mydim, ndim>(fp), HInit_(MatrixXr::Identity(k, k)), HOld_(MatrixXr::Identity(k, k)), updateH_(false){};
    //! A copy constructor: it just creates a DirectionBFGS object with the same features of rhs but it initializes the matrices.
    DirectionBFGS(const DirectionBFGS<Integrator, Integrator_noPoly, ORDER, mydim, ndim>& rhs);
    //! Clone method overridden.
    std::unique_ptr<DirectionBase<Integrator, Integrator_noPoly, ORDER, mydim, ndim>> clone() const override;
